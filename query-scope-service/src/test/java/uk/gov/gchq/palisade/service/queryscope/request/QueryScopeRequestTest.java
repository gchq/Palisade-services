...
public class QueryScopeRequestTest {
     ...
     *
     * @throws IOException throws if the {@link QueryScopeRequest} object cannot be converted to a JsonContent.
    ...
    public void groupedDependantQueryScopeRequestSerialisingAndDeserialising() throws IOException {
        ...
        ObjectContent<QueryScopeRequest> queryScopeResponseObjectContent = jacksonTester.parse(queryScopeResponseJsonContent.getJson());
        QueryScopeRequest queryScopeResponseMessageObject = queryScopeResponseObjectContent.getObject();
        ...
        assertAll("AuditSerialisingDeseralisingAndComparison",
                ...
                () -> assertAll("AuditDeserialisingComparedToObject",
                        () -> assertThat(queryScopeResponseMessageObject.getUser()).isEqualTo(queryScopeRequest.getUser()),
                        () -> assertThat(queryScopeResponseMessageObject.getContext()).isEqualTo(queryScopeRequest.getContext()),
                        () -> assertThat(queryScopeResponseMessageObject.getResource()).isEqualTo(queryScopeRequest.getResource()),
                        () -> assertThat(queryScopeResponseMessageObject.getRules()).isEqualTo(queryScopeRequest.getRules())
                ),
                () -> assertAll("ObjectComparison",
                        //The reconstructed stack trace wont be exactly the same due to different object hashes so equals is used here
                        () -> assertThat(queryScopeResponseMessageObject.equals(queryScopeRequest))
                )
        );
    }
}
