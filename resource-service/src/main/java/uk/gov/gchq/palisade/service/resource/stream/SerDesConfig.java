...
public final class SerDesConfig {
     ...
     * Used by the Rest Controller to insert requests onto the topic
     *
     * @return an appropriate value serialiser for the topic's message content (ResourceRequest)
     */
    public static Serializer<ResourceRequest> userValueSerializer() {
        return (String ignored, ResourceRequest resourceRequest) -> {
            try {
                return MAPPER.writeValueAsBytes(resourceRequest);
            } catch (IOException e) {
                throw new SerializationFailedException(SERIALIZATION_FAILED_MESSAGE + resourceRequest.toString(), e);
            }
        };
    }
     ...
     */
    public static Deserializer<String> userKeyDeserializer() {
     ...
     *
     * @return an appropriate value deserialiser for the topic's message content (ResourceRequest)
     */
    public static Deserializer<ResourceRequest> userValueDeserializer() {
        return (String ignored, byte[] resourceRequest) -> {
            try {
                return MAPPER.readValue(resourceRequest, ResourceRequest.class);
            } catch (IOException e) {
                throw new SerializationFailedException(DESERIALIZATION_FAILED_MESSAGE + new String(resourceRequest, Charset.defaultCharset()), e);
            }
        };
    }
     ...
     *
     * @return an appropriate value serialiser for the topic's message content (ResourceResponse)
     */
    public static Serializer<ResourceResponse> resourceValueSerializer() {
        return (String ignored, ResourceResponse resourceResponse) -> {
            try {
                return MAPPER.writeValueAsBytes(resourceResponse);
            } catch (JsonProcessingException e) {
                throw new SerializationFailedException(SERIALIZATION_FAILED_MESSAGE + resourceResponse.toString(), e);
            }
        };
    }
