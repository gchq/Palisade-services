...
<!---
The contents of this file are under substitution in the build process - maven's `process-resources` stage will substitute executable(dot)jar for this: services-manager-0.5.2-SNAPSHOT-exec.jar
...
```bash
java -jar -Dspring.profiles.active=static target/services-manager-0.5.2-SNAPSHOT-exec.jar
```
...
```bash
java -jar -Dspring.profiles.active=example-libs target/services-manager-0.5.2-SNAPSHOT-exec.jar
```
...
```bash
java -jar -Dspring.profiles.active=example-runner target/services-manager-0.5.2-SNAPSHOT-exec.jar
```
 * Services will start up with their cache/persistence-store prepopulated with example data
...
```bash
java -jar -Dspring.profiles.active=example-perf target/services-manager-0.5.2-SNAPSHOT-exec.jar
```
 * Services will start up with their cache/persistence-store prepopulated with example data
 ...
 * Change the above command to include the (previously unused) `performance-create-task`:
    ```bash
    java -jar -Dspring.profiles.active=example-perf target/services-manager-0.5.2-SNAPSHOT-exec.jar --manager.schedule=performance-create-task,palisade-task,performance-test-task
    ```
 ...
 * Add the `debug` profile during the manager's run command - `java -jar -Dspring.profiles.active=static,debug target/services-manager-0.5.2-SNAPSHOT-exec.jar --manager.mode=run` 
 ...
 * Add the `debug` profile and use the manager's logging command - `java -jar -Dspring.profiles.active=static,debug target/services-manager-0.5.2-SNAPSHOT-exec.jar --manager.mode=loggers`
...
When testing your new configuration, you may find the config flag useful:
 ...
 1. See what the services-manager has been given by Spring - `java -jar -Dspring.profiles.active=mynewprofile target/services-manager-0.5.2-SNAPSHOT-exec.jar --manager.mode=config` (the Java object representing the configuration should be printed to screen)  
 1. Need a little more? Also add the `debug` profile - `java -jar -Dspring.profiles.active=mynewprofile,debug target/services-manager-0.5.2-SNAPSHOT-exec.jar --manager.mode=config`  
