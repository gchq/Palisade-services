...
/**
 * Convert between Java {@link AuditErrorMessage} objects and serialised {@link String}s stored in a database
 * Simply wraps an {@link ObjectMapper}, elevating any {@link JsonProcessingException}s to {@link RuntimeException}s
 */
public class AuditErrorMessageConverter implements AttributeConverter<AuditErrorMessage, String> {
     ...
     */
    public AuditErrorMessageConverter(final ObjectMapper objectMapper) {
        this.objectMapper = Optional.ofNullable(objectMapper)
                .orElseThrow(() -> new IllegalArgumentException("objectMapper cannot be null"));
    }

    @Override
    public String convertToDatabaseColumn(final AuditErrorMessage auditErrorMessage) {
        try {
    ...
    @Override
    public AuditErrorMessage convertToEntityAttribute(final String auditErrorMessage) {
        try {
