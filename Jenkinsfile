                    ...
                    configFileProvider([configFile(fileId: "${env.CONFIG_FILE}", variable: 'MAVEN_SETTINGS')]) {
                        if (("${env.BRANCH_NAME}" == "develop") ||
                            ...
                            sh 'helm upgrade --install palisade . \
                              ...
                              --set global.persistence.classpathJars.aws.volumeHandle=${VOLUME_HANDLE},global.persistence.dataStores.aws.volumeHandle=${VOLUME_HANDLE_DATA_STORE},global.persistence.kafka.aws.volumeHandle=${VOLUME_HANDLE},global.persistence.redis.aws.volumeHandle=${VOLUME_HANDLE} \
                              --namespace dev'
                        } else {
                            sh "echo - no deploy"
                        }
