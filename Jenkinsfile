                ...
                withCredentials([string(credentialsId: 'fdf44684-3ce1-4f6b-9f1b-f2c452bd0d22', variable: 'EFS_HANDLE')]) {
                    configFileProvider([configFile(fileId: "${env.CONFIG_FILE}", variable: 'MAVEN_SETTINGS')]) {
                        if (("${env.BRANCH_NAME}" == "develop") ||
                            ...
                            sh 'helm upgrade --install palisade .  --set global.repository=${ECR_REGISTRY},global.hostname=${EGRESS_ELB},global.localMount.enabled=false,global.localMount.volumeHandle=${VOLUME_HANDLE} --namespace adam'
                        } else {
                            sh "echo - no deploy"
                        }
                    }
